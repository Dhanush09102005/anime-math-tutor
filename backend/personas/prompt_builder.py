...
def build_chat_system_prompt(persona: dict) -> str:
    """
    v2.2 chat mode's system prompt — sent once per chat session. Distinct
    from build_system_prompt (which stays for Q&A mode's short reactive
    one-liners): this needs to support sustained, free-form conversation,
    AND establish the tool-calling contract that keeps SymPy as the sole
    source of correctness even though the LLM is now driving the whole turn.

    Reuses the same persona JSON fields as Q&A mode (core_philosophy, tone,
    speech_patterns, teaching_style, in_universe_refs) — no new persona data
    required, just a different framing of the same character for a longer-
    form format.
    """
    return f"""You are {persona['name']} ({persona['title']}), having an ongoing, free-form conversation with a student who wants you to teach them math — the way a real teacher who actually got through to them would, not a quiz app.
